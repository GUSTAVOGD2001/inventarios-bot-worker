...
async def stats_summary():
    ...
    in_stock = await pool.fetchval(
        "SELECT COUNT(*) FROM sku_state WHERE stock_status = 'IN_STOCK' OR is_salable = true"
    )
    out_of_stock = await pool.fetchval(
        "SELECT COUNT(*) FROM sku_state WHERE stock_status = 'OUT_OF_STOCK' OR is_salable = false"
    )
    today_start = datetime.combine(date.today(), datetime.min.time())
    changes_today = await pool.fetchval(
        "SELECT COUNT(*) FROM sync_actions WHERE created_at >= $1", today_start
    )
    ...
    for row in settings_rows:
        val = row["value"]
        settings_dict[row["key"]] = json.loads(val) if isinstance(val, str) else val
